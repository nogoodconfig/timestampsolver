class TimestampConverter {
    ...
    createCards() {
        this.converters.forEach(converter => {
            ...
            // Generate filename from converter name
            const filename = converter.name.toLowerCase()
                .replace(/[()]/g, '')  // Remove parentheses
                .replace(/\s+/g, '')   // Remove spaces
                .replace(/\//g, '')    // Remove forward slashes
                .replace(/^unix/, 'epoch'); // Replace "unix" with "epoch" to match file naming
            const githubUrl = `https://github.com/nogoodconfig/timestampsolver/blob/main/converters/${filename}.js`;
            card.innerHTML = `
